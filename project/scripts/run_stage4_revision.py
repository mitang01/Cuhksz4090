...
def _fit_layer(config: str, model: str, variant: str, layer: str) -> dict[str, Any]:
    # region agent log
    open("/opt/cursor/logs/debug.log", "a").write(json.dumps({"hypothesisId": "C,E", "location": "run_stage4_revision.py:_fit_layer", "message": "fit worker arguments", "data": {"config_name": config.rsplit("/", 1)[-1], "model": model, "variant": variant, "layer": layer}, "timestamp": time.time() * 1000}) + "\n")
    # endregion
...
def _parallel_layers(function: Any, arguments: list[tuple[Any, ...]], workers: int) -> list:
    # region agent log
    open("/opt/cursor/logs/debug.log", "a").write(json.dumps({"hypothesisId": "C", "location": "run_stage4_revision.py:_parallel_layers", "message": "parent worker submissions", "data": {"function": function.__name__, "arguments": arguments, "workers": workers}, "timestamp": time.time() * 1000}) + "\n")
    # endregion
...
def main(argv: list[str] | None = None) -> Any:
    ...
    # region agent log
    open("/opt/cursor/logs/debug.log", "a").write(json.dumps({"hypothesisId": "A,C,E", "location": "run_stage4_revision.py:main", "message": "parsed CLI request", "data": {"command": args.command, "config_name": args.config.rsplit("/", 1)[-1], "model": getattr(args, "model", None), "variant": getattr(args, "variant", None), "layers": getattr(args, "layers", None), "layer_workers": getattr(args, "layer_workers", None)}, "timestamp": time.time() * 1000}) + "\n")
    # endregion
