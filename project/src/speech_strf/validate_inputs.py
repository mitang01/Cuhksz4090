...
def validate_manifest(frame: pd.DataFrame) -> dict:
    ...
    for record_id in duplicated["recording_id"]:
        issues.append({"recording_id": record_id, "code": "duplicate_id"})
    records = []
    for row in frame.to_dict("records"):
        record_issues = []
        if row.get("audio_matches") != 1:
            record_issues.append({"code": "missing_or_duplicate_audio"})
        if row.get("alignment_matches") != 1:
            record_issues.append({"code": "missing_or_duplicate_alignment"})
        metadata = None
        if not record_issues:
            try:
                metadata = inspect_audio(row["audio_path"])
                intervals = parse_textgrid(row["alignment_path"])
                record_issues.extend(validate_intervals(intervals, metadata["duration_seconds"]))
            except Exception as exc:
                record_issues.append({"code": "read_error", "detail": str(exc)})
...
def write_validation_report(frame: pd.DataFrame, path: str | Path) -> dict:
    report = validate_manifest(frame)
    path = Path(path)
