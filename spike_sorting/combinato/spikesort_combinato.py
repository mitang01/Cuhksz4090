...
OUTPUT_ROOT = Path("/share/home/mitan/spike_sorting/combinato")
...
REGION_CHANNEL_MAP = {
    "ATL": (64, 80),       # channels 65-80
    "HG": (80, 96),        # channels 81-96
    "VMPFC": (96, 112),    # channels 97-112
    "Amygdala": (112, 128) # channels 113-128
}
...
def ensure_uv_calibration(recording):
    """
    Combinato requires gain_to_uV and offset_to_uV properties because
    SpikeInterface writes input to H5 with return_in_uV=True.
    """
    if recording.get_channel_gains() is None:
        recording.set_channel_gains(GAIN_TO_UV)
    if recording.get_channel_offsets() is None:
        recording.set_channel_offsets(0.0)
    return recording


def ensure_single_channel_probe(recording):
    """
    SortingAnalyzer needs probe metadata for sparsity estimation.
    Attach a minimal 1-contact probe when absent.
    """
    try:
        recording.get_probegroup()
        return recording
    except Exception:
        probe = generate_linear_probe(num_elec=1, ypitch=20)
        probe.set_device_channel_indices(np.array([0], dtype="int64"))
...
def sort_one_region(
    ...
    recording_bp = ensure_uv_calibration(recording_bp)
    recording_cmr = si.common_reference(recording_bp, reference="global", operator="median")
    recording_cmr = ensure_uv_calibration(recording_cmr)
    region_spike_times = {}
    region_good_spike_times = {}
    region_meta = []
    region_channel_status = []
    total_units = 0
    requested_metrics, metric_map = get_metric_request_info()

    for local_ch_idx, chan_id in enumerate(channel_ids):
        best_ch_global_1idx = ch_start + local_ch_idx + 1
        print(f"[INFO]   Combinato channel {best_ch_global_1idx} ({chan_id})")

        recording_ch = recording_cmr.select_channels(channel_ids=[chan_id])
        recording_ch = ensure_uv_calibration(recording_ch)
        recording_ch = ensure_single_channel_probe(recording_ch)
        ch_tag = f"ch{best_ch_global_1idx:03d}"
        ch_sort_dir = sort_dir / ch_tag
        ch_analyzer_dir = analyzer_dir / ch_tag
        ch_phy_dir = phy_dir / ch_tag

        channel_status = {
            "region": region_name,
            "channel_id_0idx": int(chan_id),
            "channel_1idx": int(best_ch_global_1idx),
            "channel_tag": ch_tag,
            "status": "error",
            "num_units": 0,
            "error": None,
        ...
        try:
            sorting = si.run_sorter(
                ...
                recording=recording_ch,
                folder=str(ch_sort_dir),
                verbose=True,
                do_filter=False,
            )
            unit_ids = sorting.get_unit_ids()
            channel_status["num_units"] = int(len(unit_ids))
            if len(unit_ids) == 0:
                channel_status["status"] = "zero_units"
                print(
                    f"[WARN]   Combinato channel {best_ch_global_1idx}: "
                    "no units found. Skipping analyzer/QC."
                )
                region_channel_status.append(channel_status)
                continue
            ...
            for unit_id in unit_ids:
                ...
                np.save(
                    spike_dir / f"{region_name}_{ch_tag}_unit{unit_id}_spikes_sec.npy",
                    spike_times_s,
                )

                global_key = f"{region_name}_{ch_tag}_unit{unit_id}"
            ...
            channel_status["error"] = repr(exc)
            region_channel_status.append(channel_status)
            print(
                f"[ERROR]   Combinato channel {best_ch_global_1idx} failed: {exc}. "
                "Continuing to next channel."
            )

    n_zero = sum(1 for x in region_channel_status if x["status"] == "zero_units")
    n_error = sum(1 for x in region_channel_status if x["status"] == "error")
    print(
        f"[INFO] Region {region_name}: found {total_units} units, "
        f"QC-passing {len(region_good_spike_times)} units, "
        f"zero-unit channels={n_zero}, error channels={n_error}."
    )
    return region_spike_times, region_good_spike_times, region_meta, region_channel_status
...
def sort_one_session(session_name: str, recording_path: Path):
    ...
    session_output = OUTPUT_ROOT / f"sorting_results_{session_name}"
    ...
    for region_name, (ch_start, ch_end) in REGION_CHANNEL_MAP.items():
        region_spikes, region_good_spikes, region_meta, region_channel_status = sort_one_region(
        ...
        all_channel_status.extend(region_channel_status)
    ...
    if summary_df.empty:
        summary_df.to_csv(good_summary_csv, index=False)
    else:
    ...
    channel_status_csv = session_output / "channel_sorting_status.csv"
    pd.DataFrame(all_channel_status).to_csv(channel_status_csv, index=False)
    n_zero = sum(1 for x in all_channel_status if x["status"] == "zero_units")
    n_error = sum(1 for x in all_channel_status if x["status"] == "error")
    ...
    print(f"       - {channel_status_csv}")
    print(
        f"[INFO] Session {session_name} channel status: "
        f"zero_units={n_zero}, errors={n_error}, total_channels={len(all_channel_status)}"
    )
